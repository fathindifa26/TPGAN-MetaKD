...
from models.network import DiscriminatorStudent, GeneratorStudent
...
from models.IResNet100 import iresnet100
import onnxruntime as ort

test_time = False

# python test_KD.py --input_list validation_list.txt --resume_model 3 --epoch 5 --subdir eksp1
def parse_args():
    parser = argparse.ArgumentParser(description="bicubic")
    parser.add_argument("--input_list")
    parser.add_argument('--resume_model', help='resume_model dirname')
    parser.add_argument('--epoch', type=int, required=True, help='epoch ke berapa, untuk path save/try_{}/epoch{}/')
    parser.add_argument("--subdir", help='output_dir = save/$resume_model/epoch$epoch/test/$subdir')
...
def init_dir(args):
    ...
    os.system('mkdir -p {}'.format('/'.join([resume_dir, 'test', args.subdir, 'single'])))
    os.system('mkdir -p {}'.format('/'.join([resume_dir, 'test', args.subdir, 'grid'])))
...
if __name__ == "__main__":
    ...
    # --- Tambahan: Kelompokkan gambar per derajat ---
    ...
    # --- Load EfficientNet untuk akurasi ---
    import importlib.util
    config_dir = os.path.dirname(resume_dir)
    config_path = os.path.join(config_dir, "config_distillation.py")
    spec = importlib.util.spec_from_file_location("config_distillation", config_path)
    train_config = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(train_config)
    from models.feature_extract_network import EfficientNet_B0

    effnet = EfficientNet_B0(num_classes=train_config.G['num_classes'])
    effnet.eval()
    effnet.cuda()
    # Load checkpoint jika ada (opsional)

    # --- Inisialisasi model G ---
    G = GeneratorStudent(
        ...
        fm_mult=train_config.G['fm_mult']
    ).cuda()
    ...
    for derajat, imglist in img_by_angle.items():
        ...
        tmp_input_dir = os.path.join(resume_dir, 'test', args.subdir, f'fid_input_{derajat}')
        ...
        for idx, img in enumerate(all_real_input_imgs):
            img_vis = (img + 1) / 2
            img_pil = transforms.ToPILImage()(img_vis)
            img_pil.save(os.path.join(tmp_input_dir, f'{idx:05d}.png'))
        fid_score_val = '-'
        is_score_val = '-'
        if fid_available:
    ...
    # --- Model File Size (MB) ---
    gen_ckpts = sorted(
        [f for f in glob.glob(os.path.join(resume_dir, '*.pth')) if os.path.basename(f).startswith('Generator')],
    ...
    for step, batch in enumerate(tqdm(dataloader, desc='Testing', unit='batch')):
        ...
        for i in range(img128_fake.shape[0]):
            ...
            save_image(img128_fake_vis, '/'.join([resume_dir, 'test', args.subdir, 'single', img_name]))
            all_fake_imgs.append(img128_fake[i].detach().cpu())
    ...
    tmp_fake_dir = os.path.join(resume_dir, 'test', args.subdir, 'fid_fake')
    tmp_real_dir = os.path.join(resume_dir, 'test', args.subdir, 'fid_real')
    os.makedirs(tmp_fake_dir, exist_ok=True)
    os.makedirs(tmp_real_dir, exist_ok=True)
    for idx, img in enumerate(all_fake_imgs):
        img_vis = (img + 1) / 2
    ...
    csv_path = os.path.join(resume_dir, 'test', args.subdir, 'evaluasi.csv')
    fieldnames = [
