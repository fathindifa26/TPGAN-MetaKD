...
class LocalPathway(nn.Module):
    ...
    def forward(self,x, return_features=False):
        ...
        if return_features:
            return local_img, deconv2, [conv1, conv2, conv3]
...
class GlobalPathway(nn.Module):
    ...
    def forward(self, I128 , I64 , I32 ,  local_predict , local_feature , z, return_features=False ):
        #encoder
        ...
        if return_features:
            return decoded_img128 , decoded_img64 , decoded_img32 , fc2, [conv1, conv2, conv3, conv4]
...
class Generator(nn.Module):
    ...
    def forward( self, I128 , I64 , I32 , left_eye , right_eye , nose , mouth , z , use_dropout, return_features=False ):
        le_fake , le_fake_feature, le_feats = self.local_pathway_left_eye( left_eye, return_features=return_features)
        re_fake , re_fake_feature, re_feats = self.local_pathway_right_eye( right_eye, return_features=return_features)
        nose_fake , nose_fake_feature, nose_feats = self.local_pathway_nose( nose, return_features=return_features)
        mouth_fake , mouth_fake_feature, mouth_feats = self.local_pathway_mouth( mouth, return_features=return_features)
        ...
        I128_fake , I64_fake , I32_fake , encoder_feature, global_feats = self.global_pathway( I128 , I64 , I32 , local_vision , local_feature , z, return_features=return_features)
        encoder_predict = self.feature_predict( encoder_feature , use_dropout )

        if return_features:
            # Gabungkan semua feature intermediate
            features = {
                'global': global_feats,
                'left_eye': le_feats,
                'right_eye': re_feats,
            ...
            return I128_fake , I64_fake , I32_fake , encoder_predict , local_vision , le_fake , re_fake , nose_fake , mouth_fake , local_input, features
...
class Discriminator(nn.Module):
    def __init__(self, use_batchnorm = False , fm_mult = 1.0):
        ...
        n_fmap[1:] = emci(n_fmap[1:], fm_mult)  # hanya channel setelah input yang dikali
        for i in range( 5 ):
...
class DiscriminatorStudent(Discriminator):
    """
    Versi 'ringan' dari Discriminator TP-GAN:
    - Menggunakan fm_mult < 1 untuk mengurangi jumlah channel
    - Arsitektur sama dengan Discriminator original, hanya channel yang dikurangi
    """
    def __init__(self, use_batchnorm=False, fm_mult=0.75):
        super(DiscriminatorStudent, self).__init__(use_batchnorm=use_batchnorm, fm_mult=fm_mult)
